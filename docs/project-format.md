# Project Format Compatibility

Salieri project files use JSON and the `.salieri` extension.

Every project file must contain:

```json
{
  "formatVersion": 1,
  "song": {}
}
```

`formatVersion` is mandatory. The current supported version is `1`.

## Loading Rules

On load, Salieri:

1. parses the JSON into the versioned project envelope;
2. routes the project through the migration entry point;
3. rejects unsupported format versions with a clear error;
4. validates the resulting song before returning it to the app.

Version `1` currently needs no migration. The migration entry point exists so future versions can be upgraded in one place before validation.

## Session Model

`song.session` stores the optional clip/session layer above tracker patterns.
Older version `1` files that do not contain `session` still load with an empty
session.

Session data is intentionally reference-based:

- `clips` point at an existing pattern and a row range inside that pattern;
- `scenes` group clip slots by track;
- `clip` values inside scene slots may be omitted to represent an empty slot.

Pattern rows remain the canonical note storage. Clips do not duplicate cells.

## Stem Manifest References

`song.stemManifest` may optionally point at a project-adjacent stem manifest JSON
generated by `salieri stems scan`. Tracks may optionally carry `stem.entryId`
metadata that references one manifest entry.

These references are deliberately lightweight:

- the project loader does not require the manifest file to exist;
- missing manifest files produce app warnings, not load failures;
- stem references do not imply realtime playback or source separation;
- the manifest remains external so future sampler/audio-engine workflows can
  opt into it without changing tracker pattern data.

## Validation Rules

Loaded projects are rejected when they contain:

- an empty song title;
- BPM or LPB set to `0`;
- non-finite swing;
- no tracks, no patterns, or an empty sequence;
- duplicate track or pattern IDs;
- duplicate clip or scene IDs;
- empty track or pattern names;
- empty stem manifest paths or stem entry IDs;
- empty clip or scene names;
- MIDI channels outside `1..=16`;
- patterns with zero rows;
- pattern rows whose cell count does not match the track count;
- note pitches, velocities, or gates outside MIDI `0..=127`;
- sequence entries referencing missing pattern IDs.
- clips referencing missing patterns;
- clips with empty or out-of-bounds pattern row ranges;
- scene slots referencing missing tracks or clips;
- duplicate track slots within a scene.

Save operations run the same validation before writing. This prevents Salieri from creating a `.salieri` file it would reject on the next load.

## Fixture Policy

Committed fixtures under `fixtures/` should remain valid for the current format unless their filename explicitly describes an invalid, migration, or error case used by tests.
