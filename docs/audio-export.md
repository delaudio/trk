...
Offline sampler rendering uses plain event data: sample id, target frame, gain, pitch ratio, and velocity. The app adapts `salieri-core::sampler_events` into those events while keeping `salieri-audio` independent from project serialization and TUI state. Pattern automation is resolved before events cross into `salieri-audio`.
