...
- Curated `.trk` demo projects that exercise stable format behavior.
...
The root `.gitignore` ignores `/untitled.trk`, `/trk-midi.log`, and `/fixtures/local/`. It does not ignore `fixtures/*.trk`, so test fixtures remain trackable.
...
```bash
UPDATE_TRK_SNAPSHOTS=1 cargo test -p trk-tui --test render_snapshots
```
...
```bash
UPDATE_TRK_FIXTURES=1 cargo test -p trk-app persistence::tests::foundations_fixture_preserves_project_contracts -- --exact
UPDATE_TRK_FIXTURES=1 cargo test -p trk-ai tests::local_proposal_matches_golden_fixture -- --exact
```
