...
Salieri can export tracker patterns or the current sequence to a deterministic
Strudel-oriented JavaScript sketch:

```bash
salieri export strudel song.salieri strudel.js --pattern 1
...
- one `stack(...)` lane per Salieri track;
...
The workflow is export-only. It does not change the canonical `.salieri` project
