...
| Ableton push/pull/clear | Source Live bridge CLI and desktop expert | Implemented / Partial | Tracker has dry-run `:ableton push`, `:ableton pull`, and `:ableton clear` plans that map clip launcher scenes/tracks to Live Session View without requiring Live. Real transport remains future optional configuration. | #89, delaudio/salieri#7 |
