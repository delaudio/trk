...
This page tracks trk against the local Polyend Tracker Mini
Essentials reference supplied with issue #74. The goal is workflow parity, not
hardware or UI cloning: trk stays a host-based terminal tracker with a
portable `.trk` project format.
...
| Area | Capability | Status | trk mapping and rationale | Owner |
| --- | --- | --- | --- | --- |
| Hardware | Battery, screen, physical controls, SD card, bundled adapters | Host OS | trk runs on the host. Power, display, storage, USB, audio, and MIDI ports are provided by the operating system and connected devices. | n/a |
...
| Project | Portable project file with patterns, tracks, sequence, samples, mixer, and runtime-neutral state | Supported | `.trk` is the canonical project format with validation and fixture coverage. Hardware card layout is mapped to workspace/project library paths. | existing |
...
| Configuration | Tracker Mini-style function-button binding | Partial | trk has stable commands, keymaps, focus targets, and command palette recents. Dedicated Fn-button pages map to configurable keybindings and commands rather than fixed hardware buttons. | existing |
...
| Tracker FX | Two step FX columns and core timing/position/value commands | Partial | trk has FX1/FX2 fields, supported parser commands, and playback behavior for implemented effects. Deferred command families are explicit diagnostics, not silent parity claims. | existing |
...
| Instruments | Track-level sample assignment and sample-backed instrument data | Supported | Current projects persist independent sample-backed instrument slots, track assignments, root note, gain, playback settings, envelope data, and future key/velocity zone metadata. | #77 |
| Instruments | Reusable instrument preset files | Supported | `:preset instrument save/show/load` exports and imports portable `trk.instrument-preset.v1` files without deleting existing project resources. | #77, #93 |
| Sampler | WAV sample browser, preview, assignment, playback range, gain, root note, and envelope basics | Partial | Sampler foundations exist and are tracked in the sampler parity roadmap. Loop crossfade, choking, and sampler-local modulation remain incomplete. | [sampler parity](sampler-parity-roadmap.md) |
| Sampler | One-shot, forward loop, backward loop, ping-pong, reverse, slice, beat-slice, wavetable, and granular modes | Partial | One-shot, forward loop, backward loop, ping-pong loop, and reverse rendering are implemented for realtime and offline paths. Slice, beat-slice, wavetable, and granular modes remain deferred. | #76 |
| Sampling | System audio input sample recorder with trim/crop/save/load | Partial | `:sample recorder inputs|capture|trim|save|save-load` records bounded WAV takes from available system inputs, crops them, saves them, and loads them into the sampler; recorder state transitions are covered with fake input tests for CI. | #78 |
...
| Performance | Momentary performance effects targeting tracks/patterns | Partial | `:performance slot/punch/release` supports temporary track gain, pan, and sample-gain punch-ins on the playback clone, with release restoring saved project state. Broader effect slots and preset persistence remain future work. | #83 |
| Mixer and routing | Track mix, pan, mute/solo, sample gain automation, and native device chains | Partial | Mixer and native DSP foundations exist for realtime/offline paths, including send/return routing. Broader graphical mixer and metering remain future work. | existing |
| Mixer and routing | Delay/reverb sends and master processing chain | Supported | Delay/reverb send buses and master chains are serializable and shared by realtime/offline sampler render paths; unsupported future devices remain outside the current native catalog. | #84 |
| MIDI | MIDI output, fake MIDI tests, MIDI logging, input parsing, clock/transport routing, note/CC gates, channel filters, middle-C, and sync-delay settings | Partial | trk is MIDI-first and persists Tracker Mini-style MIDI routing controls. Note input/output and fake clock/transport behavior are enforced; CC routing is filtered but awaits a stable automation/learn target. | #82 |
...
| DAW integration | Ableton-style clip launcher and Live bridge | Partial | Clip launcher scenes are modeled and editable as an optional scene x track workflow. Live push/pull/clear now has a dry-run terminal bridge; real Live transport remains opt-in future work. | #87, #89 |
...
| Research | Style analysis, reports, dossiers, lyrics, live-coding export, composition graph | Partial | Text notes, lyric lines, cue markers, compact annotation reports, deterministic style analysis/compare, Strudel text export, local report/critique/revision workflows, and first-pass composition graph sequence compilation are implemented as optional workflows. Broader dossier, clip graph, and Ableton graph targets remain deferred. | #90, #92, #96, #97, #98, #99 |

## Hardware-to-host mapping

| Tracker Mini concept | trk equivalent |
| --- | --- |
