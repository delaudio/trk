...
| Pattern editor | Fill, invert, expand, shrink, duplicate, and richer pattern/selection operations | Planned | Basic pattern and selection operations exist, but the broader Tracker Mini pattern-operation set needs focused commands and undo tests. | #86 |
| Pattern editor | Render selection into a reusable sample | Planned | Requires internal audio rendering of selected rows/tracks and immediate sample registration. | #79 |
...
| Instruments | Reusable instrument preset files | Planned | Salieri needs portable preset import/export once instrument slots are independent from tracks. | #77 |
...
| Sampling | Internal render/bounce workflows | Planned | Maps Tracker Mini internal render workflows to offline render plus sample registration. MIDI-only external instruments remain explicit non-rendered sources. | #79, #95 |
...
| Research | Style analysis, reports, dossiers, lyrics, live-coding export, composition graph | Deferred | These are post-MVP Salieri workflows, not Tracker Mini parity blockers. | #90, #92, #96, #97, #98, #99 |
