...
| Sampling | Internal render/bounce workflows | Partial | Render plans and deterministic per-track stem export exist for internal sampler/native audio. Rendering a selection directly into a reusable sample remains tracked separately. MIDI-only external instruments remain explicit non-rendered sources. | #79, #95 |
