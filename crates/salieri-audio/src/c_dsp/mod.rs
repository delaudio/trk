...
//!
//! This module is intentionally feature-gated and only wraps Salieri-owned or
...
pub struct CNativeGainModule {
    ...
    ffi_state: ffi::SalieriCGainState,
...
impl CNativeGainModule {
    ...
    pub fn new(spec: CNativeGainSpec) -> Self {
        let mut module = Self {
            spec,
            default: spec,
            ffi_state: ffi::SalieriCGainState::default(),
            prepared: None,
        };
    ...
    pub fn process_in_place(&mut self, data: &mut [f32]) -> Result<(), AudioExportError> {
        ...
        match status {
            ffi::SALI_SUCCESS => Ok(()),
            ffi::SALI_ERR_FRAME_OR_CHANNEL_MISMATCH => Err(AudioExportError::InvalidBufferLength {
                expected: prepared.max_block_frames.saturating_mul(channels),
                actual: data.len(),
            }),
    ...
    fn reset_ffi_state(&mut self) {
        unsafe {
            ffi::salieri_c_gain_reset(&mut self.ffi_state);
        }
    ...
    fn apply_state_to_ffi(&mut self) -> Result<(), AudioExportError> {
        ...
        let status =
            unsafe { ffi::salieri_c_gain_set_gain(&mut self.ffi_state, self.spec.state.gain) };
        if status == ffi::SALI_SUCCESS {
            Ok(())
        } else {
...
mod ffi {
    ...
    pub const SALI_SUCCESS: c_int = 0;
    pub const SALI_ERR_FRAME_OR_CHANNEL_MISMATCH: c_int = -3;

    #[repr(C)]
    pub struct SalieriCGainState {
        gain: c_float,
        sample_rate: c_uint,
        channels: u16,
    ...
    impl Default for SalieriCGainState {
        fn default() -> Self {
            Self {
                gain: 1.0,
                sample_rate: 0,
    ...
    unsafe extern "C" {
        pub fn salieri_c_gain_reset(state: *mut SalieriCGainState);
        pub fn salieri_c_gain_prepare(
            state: *mut SalieriCGainState,
            sample_rate: c_uint,
            channels: u16,
            max_block_frames: usize,
        ) -> c_int;
        pub fn salieri_c_gain_set_gain(state: *mut SalieriCGainState, gain: c_float) -> c_int;
        pub fn salieri_c_gain_process(
            state: *mut SalieriCGainState,
