...
impl Keymap {
    ...
    pub fn command_for(&self, mode: KeymapMode, event: &KeyEvent) -> Option<TrkCommand> {
...
fn parse_command(value: &str) -> Result<TrkCommand, String> {
    let value = command_text(value);
    match TrkCommand::parse(&value) {
        Ok(Some(command)) => {
