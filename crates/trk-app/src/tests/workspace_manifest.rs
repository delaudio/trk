...
fn workspace_init_writes_portable_manifest_and_default_roots() {
    ...
    let manifest_path = root.join(".trk-workspace.json");
    ...
    assert_eq!(value["schema"], "trk.workspace.v1");
...
fn workspace_index_counts_artifact_roots_from_one_model() {
    ...
    let mut app = App::default();
    type_command(&mut app, &format!("workspace init {}", root.display()));
    write_file(root.join("projects/song.trk"), "{}");
...
fn workspace_trash_and_restore_are_non_destructive_moves() {
    ...
    let mut app = App::default();
    type_command(&mut app, &format!("workspace init {}", root.display()));
    let project = root.join("projects/song.trk");
    write_file(&project, "{}");

    type_command(
        &mut app,
        &format!("workspace trash {} projects/song.trk", root.display()),
    );

    assert!(!project.exists());
    assert!(root.join(".trk-trash/song.trk").exists());
    let json = std::fs::read_to_string(root.join(".trk-workspace.json")).expect("manifest");
    let value = serde_json::from_str::<serde_json::Value>(&json).expect("valid json");
    assert_eq!(value["trashRecords"][0]["original"], "projects/song.trk");
    assert_eq!(value["trashRecords"][0]["trashed"], ".trk-trash/song.trk");

    type_command(
        &mut app,
        &format!("workspace restore {} projects/song.trk", root.display()),
    );
...
fn workspace_test_dir(label: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("trk-workspace-{label}-{}", std::process::id()))
}
