...
fn command_mode_adds_and_removes_sequence_positions() {
    ...
    assert_eq!(app.song.sequence, vec![trk_core::PatternId(2)]);
...
fn command_mode_reports_sequence_add_pattern_out_of_range() {
    ...
    assert_eq!(app.song.sequence, vec![trk_core::PatternId(1)]);
    assert!(!app.dirty);
    assert_eq!(
        app.notification.as_ref().map(|n| n.message.as_str()),
...
fn command_mode_duplicates_sets_and_moves_sequence_positions() {
    ...
    assert_eq!(
        app.song.sequence,
        vec![
            trk_core::PatternId(1),
            trk_core::PatternId(2),
            trk_core::PatternId(2),
            trk_core::PatternId(3)
        ]
    );
    assert_eq!(
        app.notification.as_ref().map(|n| n.message.as_str()),
    ...
    assert_eq!(app.song.sequence[0], trk_core::PatternId(3));
    assert_eq!(
        app.notification.as_ref().map(|n| n.message.as_str()),
    ...
    app.handle_key(KeyEvent::new(KeyCode::Char('z'), KeyModifiers::CONTROL));
    assert_eq!(app.song.sequence[1], trk_core::PatternId(2));
}
...
fn keyboard_sequence_shortcuts_edit_selected_position() {
    ...
    assert_eq!(app.song.sequence[2], trk_core::PatternId(3));
