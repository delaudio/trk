...
fn region(
    map: &InteractionMap,
    id: trk_tui::InteractionRegionId,
    predicate: impl Fn(InteractionPayload) -> bool,
) -> trk_tui::InteractionRegion {
    map.regions()
        .iter()
        .copied()
...
fn point(region: trk_tui::InteractionRegion) -> (u16, u16) {
    (
...
fn inert_neighbor(
    map: &InteractionMap,
    target: trk_tui::InteractionRegion,
    size: (u16, u16),
) -> (u16, u16) {
    let (width, height) = size;
    let candidates = [
        ...
        project_browser_view: Some(AppProjectBrowserView {
            ...
            entries: (0..40)
                .map(|index| AppProjectBrowserEntry {
                    path: PathBuf::from(format!("/projects/{index:02}.trk")),
                    name: format!("{index:02}.trk"),
