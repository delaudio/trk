...
fn sampler_view_opens_without_sample_and_loads_wav_from_command() {
    ...
    let path = std::env::temp_dir().join(format!("trk-sampler-view-{}.wav", std::process::id()));
    std::fs::write(
        &path,
...
fn sampler_commands_assign_list_and_unassign_loaded_sample() {
    let mut app = App::default();
    let path = std::env::temp_dir().join(format!("trk-sampler-assign-{}.wav", std::process::id()));
...
fn sample_render_selection_writes_wav_reference_and_can_assign_track() {
    ...
    let sample_path = std::env::temp_dir().join(format!(
        "trk-render-selection-source-{}.wav",
        std::process::id()
    ));
    let output_path = std::env::temp_dir().join(format!(
        "trk-render-selection-bounce-{}.wav",
        std::process::id()
    ));
    std::fs::write(
        &sample_path,
...
fn sample_render_selection_failure_does_not_mutate_project() {
    ...
    let missing_dir = std::env::temp_dir().join(format!(
        "trk-render-selection-missing-dir-{}",
        std::process::id()
    ));
...
fn sample_recorder_save_load_trims_and_assigns_recorded_wav() {
    let mut app = App::default();
    let output_path =
        std::env::temp_dir().join(format!("trk-recorder-save-load-{}.wav", std::process::id()));
    app.load_fake_sample_recording_for_test(trk_audio::RenderedAudio {
        sample_rate: 44_100,
        channels: 1,
        frames: 4,
...
fn sampler_commands_edit_playback_settings() {
    let mut app = App::default();
    let path =
        std::env::temp_dir().join(format!("trk-sampler-settings-{}.wav", std::process::id()));
    std::fs::write(
        &path,
        wav_pcm16_bytes(44_100, 1, &[0, i16::MAX, i16::MIN, 16_384]),
    )
...
fn sampler_view_keyboard_controls_edit_envelope() {
    let mut app = App::default();
    let path =
        std::env::temp_dir().join(format!("trk-sampler-envelope-{}.wav", std::process::id()));
    std::fs::write(
        &path,
        wav_pcm16_bytes(44_100, 1, &[0, i16::MAX, i16::MIN, 16_384]),
    )
...
fn sampler_commands_replace_unload_and_cleanup_references() {
    let mut app = App::default();
    let first_path =
        std::env::temp_dir().join(format!("trk-sampler-first-{}.wav", std::process::id()));
    let second_path =
        std::env::temp_dir().join(format!("trk-sampler-second-{}.wav", std::process::id()));
...
fn in_app_sample_browser_previews_and_loads_wav_files() {
    let mut app = App::default();
    let dir =
        std::env::temp_dir().join(format!("trk-in-app-sample-browser-{}", std::process::id()));
...
fn in_app_sample_browser_assigns_selected_wav_with_shortcut() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "trk-in-app-sample-browser-assign-{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&dir).expect("create sample dir");
...
fn in_app_sample_browser_reports_unsupported_files() {
    let mut app = App::default();
    let dir = std::env::temp_dir().join(format!(
        "trk-unsupported-sample-browser-{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&dir).expect("create sample dir");
...
fn project_browser_discovers_projects_and_persists_recent_projects() {
    let dir = std::env::temp_dir().join(format!(
        "trk-project-browser-discovery-{}",
        std::process::id()
    ));
    let recent_file = dir.join("recent-projects.json");
    let project_path = dir.join("valid.trk");
    let missing_path = dir.join("missing.trk");
    let invalid_path = dir.join("invalid.trk");
    ...
    assert_eq!(entries[0].name, "valid.trk");
    assert!(entries[0].detail.contains("Discovery Test"));
    assert_eq!(entries[1].kind, ProjectBrowserEntryKind::MissingProject);
    assert_eq!(entries[1].name, "missing.trk");
    assert!(entries.iter().any(|entry| entry.name == "invalid.trk"
...
fn project_browser_opens_valid_project_and_records_recent() {
    let dir = std::env::temp_dir().join(format!("trk-project-browser-open-{}", std::process::id()));
    let recent_file = dir.join("recent-projects.json");
    let project_path = dir.join("opened.trk");
    std::fs::create_dir_all(&dir).expect("create project dir");
    let mut song = Song::empty();
...
fn project_browser_requires_confirmation_before_discarding_dirty_song() {
    let dir =
        std::env::temp_dir().join(format!("trk-project-browser-dirty-{}", std::process::id()));
    let recent_file = dir.join("recent-projects.json");
    ...
    let selected = run_external_sample_browser(
        &SampleBrowserConfig {
            chooser_command: Some(
                "printf '%s\n' \"$TRK_SAMPLE_START_DIR/pick.wav\" > \"$TRK_CHOOSER_FILE\""
                    .to_string(),
            ),
            start_dir: Some(PathBuf::from("Samples")),
        },
