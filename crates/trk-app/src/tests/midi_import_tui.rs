...
fn command_mode_imports_midi_into_dirty_tui_project() {
    let base = std::env::temp_dir().join(format!("trk-tui-midi-import-{}", std::process::id()));
    ...
    assert_eq!(
        ...
        trk_core::model::DEFAULT_PATTERN_LEN
    );
    assert_eq!(
        app.song
            .pattern(0)
...
fn command_mode_imports_midi_with_shell_escaped_path() {
    let base = std::env::temp_dir()
        .join(format!("trk tui midi import {}", std::process::id()))
