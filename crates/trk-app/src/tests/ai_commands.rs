...
fn command_mode_ai_proposal_reports_missing_command_provider_requirements() {
    let missing_env = format!("TRK_TEST_MISSING_AI_TOKEN_{}", std::process::id());
    let mut app = App::new(AppConfig {
        ai: config::AiConfig {
            provider: config::AiProviderKind::Command,
            model: "codex-cli".to_string(),
            command_path: Some("definitely-missing-trk-ai-command".to_string()),
    ...
    assert!(notification
        .message
        .contains("command binary not found: definitely-missing-trk-ai-command"));
...
fn ai_session_test_path(label: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!(
        "trk-ai-command-session-{label}-{}.json",
        std::process::id()
    ))
}

fn ai_guidance_test_dir(label: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("trk-ai-guidance-{label}-{}", std::process::id()))
}
