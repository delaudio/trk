...
impl App {
    ...
    pub(crate) fn execute_typed_command(&mut self, parsed: TrkCommand) {
        match parsed {
            ...
            TrkCommand::Layout(command) => self.handle_layout_command(command),
            TrkCommand::Quit { force: false } => self.request_quit(false),
            TrkCommand::Quit { force: true } => self.force_quit(),
            TrkCommand::Write(path) => match path {
                Some(path) => self.save_as(path),
                None => self.save(),
            },
            TrkCommand::SaveAs(path) => {
                self.save_as(path);
            }
            TrkCommand::WriteQuit => {
                self.save_and_quit();
            }
            TrkCommand::SetBpm(value) => {
                self.dispatch_intent(AppIntent::Parameter(ParameterIntent::SetBpm(value)));
                self.notify_success(format!("BPM set to {value}"));
            }
            TrkCommand::SetLinesPerBeat(value) => {
            ...
            TrkCommand::Play(command) => match command {
