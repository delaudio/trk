...
pub(crate) fn run_export_plan(args: &RenderPlanArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing render-plan input path: usage is trk export plan INPUT [OUTPUT.json]")?;
...
pub(crate) fn run_export_stems(args: &RenderStemsArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing stem input path: usage is trk export stems INPUT OUT_DIR")?;
    let output_dir = args
        .output_dir
        .as_deref()
        .context("missing stem output directory: usage is trk export stems INPUT OUT_DIR")?;
    let song = load_project(input_path)?;
