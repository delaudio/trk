...
async function poll(){if(inflight)return;inflight=new AbortController();const timer=setTimeout(()=>inflight.abort(),220);try{const r=await fetch("/api/state",{cache:"no-store",signal:inflight.signal});if(!r.ok)throw new Error(`state ${r.status}`);const next=await r.json();if(next.version!==1)throw new Error("unsupported state version");state=next;lastSeen=performance.now();setConnection(true,"live · 20 Hz");if(next.revision!==lastRevision){lastRevision=next.revision;syncDom()}$("error").textContent=""}catch(e){setConnection(false,"disconnected");$("error").textContent=e.name==="AbortError"?"state timeout":e.message}finally{clearTimeout(timer);inflight=null;setTimeout(poll,50)}}
