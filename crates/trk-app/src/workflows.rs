...
pub(crate) fn run_export_audio(args: &AudioExportArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing export input path: usage is trk export audio INPUT OUTPUT")?;
    let output_path = args
        .output_path
        .as_deref()
        .context("missing export output path: usage is trk export audio INPUT OUTPUT")?;
...
pub(crate) fn run_import_xrns(args: &ImportXrnsArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing import input path: usage is trk import xrns INPUT OUTPUT")?;
    let output_path = args
        .output_path
        .as_deref()
        .context("missing import output path: usage is trk import xrns INPUT OUTPUT")?;
...
pub(crate) fn run_import_midi(args: &ImportMidiArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing import input path: usage is trk import midi INPUT OUTPUT")?;
    let output_path = args
        .output_path
        .as_deref()
        .context("missing import output path: usage is trk import midi INPUT OUTPUT")?;
    let bytes = fs::read(input_path)
...
pub(crate) fn convert_sample_payload_to_wav(
    ...
    let temp_input = output_path.with_extension(format!("trk-import.{format}"));
...
pub(crate) fn inspect_sample(args: &SampleInspectArgs) -> Result<SampleInspection> {
    let path = args
        .path
        .as_deref()
        .context("missing sample path: usage is trk sample inspect FILE")?;
...
) -> Result<trk_audio::RenderedAudio> {
