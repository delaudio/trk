...
fn cli_analyze_and_compare_write_text_and_json_outputs() {
    let base = std::env::temp_dir().join(format!("salieri-style-cli-{}", std::process::id()));
...
fn tui_analyze_and_compare_append_reports_without_mutating() {
    let base = std::env::temp_dir().join(format!("salieri-style-tui-{}", std::process::id()));
    let other_path = base.with_extension("salieri");
