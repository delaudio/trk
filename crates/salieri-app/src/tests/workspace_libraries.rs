...
fn sample_browser_uses_workspace_sample_library_by_default() {
    let dir =
        std::env::temp_dir().join(format!("salieri-workspace-samples-{}", std::process::id()));
...
fn project_browser_uses_workspace_project_library_by_default() {
    let dir =
        std::env::temp_dir().join(format!("salieri-workspace-projects-{}", std::process::id()));
    let project_path = dir.join("library.salieri");
    ...
    assert!(browser
        .entries
        .iter()
        .any(|entry| entry.name == "library.salieri"));
...
fn project_browser_prefers_workspace_library_over_current_project_parent() {
    let library_dir =
        std::env::temp_dir().join(format!("salieri-workspace-priority-{}", std::process::id()));
    let current_dir =
        std::env::temp_dir().join(format!("salieri-current-project-{}", std::process::id()));
    std::fs::create_dir_all(&library_dir).expect("create library dir");
    std::fs::create_dir_all(&current_dir).expect("create current dir");
    let library_project = library_dir.join("library.salieri");
    let current_project = current_dir.join("current.salieri");
