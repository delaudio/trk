...
pub(crate) fn run_import_musicxml(args: &ImportMusicXmlArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing import input path: usage is salieri import musicxml INPUT OUTPUT")?;
    let output_path = args
        .output_path
        .as_deref()
        .context("missing import output path: usage is salieri import musicxml INPUT OUTPUT")?;
...
pub(crate) fn run_export_musicxml(args: &MusicXmlExportArgs) -> Result<()> {
    let input_path = args
        .input_path
        .as_deref()
        .context("missing export input path: usage is salieri export musicxml INPUT [OUTPUT]")?;
...
pub(crate) fn run_validate_round_trip(args: &RoundTripValidationArgs) -> Result<()> {
    let input_path = args.input_path.as_deref().context(
        "missing validation input path: usage is salieri validate roundtrip INPUT [OUTPUT]",
    )?;
    let song = load_project(input_path)?;
...
fn validate_midi_round_trip(song: &Song, pattern: usize) -> Result<MidiRoundTripReport> {
    ...
    let bytes = salieri_interop::export_pattern_smf(
        song,
        MidiExportOptions {
            pattern,
...
fn print_musicxml_diagnostics(diagnostics: &[salieri_interop::MusicXmlDiagnostic]) {
