...
fn legacy_module_diagnostics() -> Vec<TrackerModuleDiagnostic> {
    vec![
        tracker_module_diagnostic(
            ...
            "legacy tracker tick tempo, speed changes, and row effects do not map losslessly to trk row timing",
        ),
        tracker_module_diagnostic(
            TrackerModuleDiagnosticKind::UnsupportedEffectMemory,
            "legacy tracker effect memory and per-channel playback state are not represented in trk pattern cells",
        ),
        tracker_module_diagnostic(
            TrackerModuleDiagnosticKind::EffectDecodeIncomplete,
