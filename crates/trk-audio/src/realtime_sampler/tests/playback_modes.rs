...
#[test]
fn realtime_and_offline_match_for_normalized_window_and_envelope() {
    let playback = AudioSamplerPlaybackSettings {
        start_frame: Some(1),
        end_frame: Some(4),
    ...
    let realtime_rendered = realtime.render(4);
    let offline_rendered = render_sampler_events(
        &[OfflineSamplerSample {
            sample_id: 7,
            buffer: sample,
    ...
    assert_eq!(realtime_rendered.data, offline_rendered.data);
    assert_eq!(realtime_rendered.data, vec![0.0, 1.0, 1.5, 0.0]);
    assert_eq!(realtime.active_voice_count(), 0);
}
